...
The output of the above code is `r m_rel`, which indicates that the RWA scale as a whole is reliable, assuming that the underlying $\theta$ distribution follows the Gaussian or normal distribution [@chalmersAnswerDifferenceEmpirical2019]. Since reliability can vary across different levels of $\theta$, readers can also visualize the scale's reliability given the levels of $\theta$ by running the function of *ggmirt* package, as follows:
...
Second, *mirt* can also calculate the RWA scale overall reliability using $\theta$ predicted by the model. To compute empirical reliability, one needs to calculate estimated $\theta$ value of each participant first, and then, calculate reliability based on these values. To do this, readers can run these two lines of codes:
...
The `fscores()` function is used to compute the estimated $\theta$ value for each participant predicted by the `fit` model. The `full.scores.SE` is set to `TRUE` because the package uses the $\theta$ value and standard error of each participant (*SE*~$\theta$~) to compute the empirical reliability. We store the estimated $\theta$ and its *SE*~$\theta$~ in a new data frame, namely `theta_se`, and then run `empirical_rxx()` function to calculate empirical reliability. The output of these codes is `r e_rel`, which is very close to marginal reliability. However, it is very important to note that while marginal and empirical reliability aim to quantify the scale's overall reliability, they base the calculations on different assumptions. Marginal reliability assumes that $\theta$ is normally distributed, and empirical reliability uses $\theta$ distribution predicted by the model, which still contains measurement errors but more realistic.

It is very important to note that our model has a poor fit, as we showed in step 4. In practice, we recommend that readers address the model fitting issues before interpreting reliability or drawing substantive conclusions based on these estimates. 
...
The output of `omega()` function includes many information but we only focus on Cronbach's $\alpha$ and McDonald's $\omega$~h~. Both Cronbach's $\alpha$ ($\alpha$ = `r alpha`) and McDonald's $\omega$ ($\omega$~h~ = `r oh`) shows that the RWA scale is overall internally consistent. 
